...
class MVANetPredictor(object):
    ...
    def __call__(self, image: Union[PilImage, List[PilImage]], output_type: OutputType = "rgba") -> Union[PilImage, List[PilImage]]:
        if isinstance(image, list):
            return self.batch_predict(image, output_type)
        else:
            return self.single_predict(image, output_type)
    
    def single_predict(self, image: PilImage, output_type: OutputType = "rgba") -> PilImage:
    ...
    def batch_predict(self, images: List[PilImage], output_type: OutputType = "rgba") -> List[PilImage]:
